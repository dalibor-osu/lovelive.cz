    ...
    <div class="post__container-card px-4 py-2 flex justify-between">
      <div class="flex gap-3 items-center">
        <button type="button" class="post__like has-tooltip hover:text-[#df067f] hover:bg-white rounded-2xl p-1">
          <svg xmlns="http://www.w3.org/2000/svg" :fill="post.liked ? 'white' : 'none'" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
               class="w-6 h-6"
               @click="like">
        ...
        <span class="post__count-likes text-lg">{{ post.likeCount }}</span>
      </div>
